 ...
 */
public class FindTheRunningMaiden {
    public static void main(String[] args) throws Exception{
            try{
                ...
                for(int a_i=0; a_i < n; a_i++){
                    ...
                    if(a[a_i] < -1 && a[a_i] > 100000) {
                        throw new Exception();
                    }
                    if(a[a_i] > max) {
                        max = a[a_i];
                    }
                    System.out.println(findTheMaiden(a,a_i+1,max+1));
                }
    ...
    static double findTheMaiden(int[] arr, int len,int max) {
        ...
        int mid = finalArrCount/2;
        if(finalArrCount%2 == 0) {
            return ((double) (finalArr[mid]+finalArr[mid-1]))/2d;
        } else {
